...
const Toast = ({ message, type, onClose }) => (
  ...
  >
    {/* Changed success icon color to Teal */}
    {type === 'success' ? <CheckCircle2 size={18} className="text-teal-500" /> : <AlertCircle size={18} />}
    ...
    <div className="min-h-screen bg-[#FDFBF7] font-montserrat text-stone-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <h1 className="font-marcellus text-4xl text-stone-900 mb-2">My Saved Items</h1>
          ...
          <div className="relative group">
            ...
            <div className="flex items-center gap-2 border border-stone-300 rounded-sm px-4 py-2 bg-white hover:border-teal-700 transition-colors">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white border border-stone-100 rounded-sm">
            ...
            <button onClick={() => dispatch(fetchWishlist())} className="mt-4 text-xs font-bold uppercase tracking-widest border-b border-stone-900 pb-1 hover:text-teal-700 hover:border-teal-700">Try Again</button>
          </div>
          ...
          >
            ...
            <p className="text-stone-500 font-light mb-8 max-w-md mx-auto">
              Save your favorite finds here to view or buy them later.
            </p>
            <Link
              to="/products"
              // Changed button hover to Teal
              className="inline-flex items-center gap-2 bg-stone-900 text-white px-8 py-3 rounded-sm text-xs font-bold uppercase tracking-widest hover:bg-teal-700 transition-colors"
            >
                ...
                >
                  <div className="relative overflow-hidden aspect-[3/4]">
                    ...
                    <div className="absolute top-2 left-2">
                        {/* Changed badge bg to Teal */}
                        <span className="bg-teal-700/90 backdrop-blur text-white text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-sm">
                            {item.category}
                        </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <Link to={`/products/${item._id}`} className="mb-2 block">
                      ...
                      <h3 className="font-marcellus text-lg text-stone-900 leading-tight group-hover:text-teal-700 transition-colors">
                        {item.name}
                      </h3>
                    </Link>
                    
                    <div className="flex items-center gap-2 mb-4">
                        {/* Changed star color to Teal */}
        ...
        {showConfirmModal && (
          <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <motion.div 
                ...
                className="bg-white rounded-sm shadow-2xl max-w-sm w-full p-6 text-center border-t-4 border-teal-600"
            >
              {/* Changed icon bg to Teal */}
