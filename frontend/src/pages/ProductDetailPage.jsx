...
import { 
  ...
  ShoppingBag, ArrowLeft, Share2, ShieldCheck 
} from 'lucide-react';
...
// Placeholder image for the promo section
const promoImage = 'https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?auto=format&fit=crop&q=80&w=1200';
...
const Toast = ({ message, type, onClose }) => (
  ...
  >
    {/* Changed icon color to Teal */}
    {type === 'success' ? <Check size={16} className="text-teal-500" /> : <Heart size={16} />}
  ...
  return (
    <div className="border-b border-stone-200">
      ...
      >
        ...
        <span className="font-marcellus text-sm text-stone-800 group-hover:text-teal-700 transition-colors">{title}</span>
  ...
  <div className="flex gap-0.5">
    {[...Array(5)].map((_, i) => (
      <Star 
        key={i} 
        size={14} 
        // Changed star colors to Teal/Stone
        className={i < Math.floor(rating) ? "fill-teal-700 text-teal-700" : "text-stone-300"} 
      />
    ))}
  </div>
);
  ...
  return (
    <div className="border-t border-stone-200 mt-20 pt-16" id="reviews">
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
        <div>
          <h2 className="font-marcellus text-2xl text-stone-900 mb-2">Seller Feedback</h2>
        ...
        {!hasUserReviewed && userInfo && !showForm && (
          <button 
            ...
            className="border-b border-stone-900 text-stone-900 pb-1 text-xs font-bold uppercase tracking-widest hover:text-teal-700 hover:border-teal-700 transition-colors"
          >
            Leave Feedback
          </button>
        )}
          ...
          >
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} type="button" onClick={() => setRating(star)}>
                    {/* Changed star colors to Teal */}
                    <Star size={20} className={star <= rating ? "fill-teal-700 text-teal-700" : "text-stone-300"} />
                  </button>
                ))}
              </div>
            </div>
            <div className="mb-4">
              ...
              <textarea 
                ...
                className="w-full p-3 border border-stone-200 rounded-sm text-sm focus:outline-none focus:border-teal-700" 
                rows="4" 
                required 
              />
            </div>
            <div className="flex gap-4">
              {/* Changed button hover to Teal */}
              <button type="submit" disabled={loading} className="bg-stone-900 text-white px-6 py-3 text-xs font-bold uppercase tracking-widest hover:bg-teal-700 transition-colors disabled:opacity-50">
      ...
      <div className="space-y-8">
        {reviews.length === 0 ? (
          <p className="text-stone-500 italic text-sm">No feedback yet. Be the first to review this item.</p>
        ) : (
          reviews.map((review) => (
    ...
    <div className="bg-[#FDFBF7] font-montserrat text-stone-800">
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
            ...
            <Link to="/products" className="text-xs font-bold uppercase tracking-widest text-stone-500 hover:text-teal-700 flex items-center gap-2 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="mb-6 border-b border-stone-200 pb-6">
              ...
              <span className="text-teal-700 text-xs font-bold uppercase tracking-[0.2em] mb-2 block">{product.category}</span>
            ...
            <div className="flex gap-4 mb-10">
              <button
                ...
                // Changed hover bg to Teal
                className="flex-1 bg-stone-900 text-white py-4 text-xs font-bold uppercase tracking-[0.2em] hover:bg-teal-700 transition-all disabled:bg-stone-400 flex items-center justify-center gap-2 shadow-xl shadow-stone-900/10"
              >
                {isAddingToCart ? <Loader2 className="animate-spin" size={16} /> : <ShoppingBag size={16} />}
                {isAddingToCart ? 'Processing...' : 'Add to Bag'}
              </button>
              <button
            ...
            <div className="space-y-2">
                ...
                <DetailsAccordion title="Condition & Specs">
                    This item has been inspected for quality. <br/>
                    <Link to="/about" className="text-teal-700 hover:underline flex items-center gap-1 mt-2 font-medium"><ShieldCheck size={14}/> Read our verification promise</Link>
                </DetailsAccordion>
                <DetailsAccordion title="Buyer Protection">
                    Your payment is held securely until you confirm the item is as described. Returns accepted within 3 days of delivery if listing was inaccurate.
                </DetailsAccordion>
            </div>
          </div>
        </div>
        ...
        <div className="mt-24 relative rounded-sm overflow-hidden h-80 group">
            ...
            <img src={promoImage} alt="Eco Friendly" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center text-white p-6">
                <h2 className="font-marcellus text-4xl md:text-5xl mb-4">Eco-Conscious Living</h2>
                <p className="text-stone-200 max-w-lg mb-8 font-light">Every pre-loved item you buy extends its life and reduces waste.</p>
                {/* Changed button hover to Teal */}
                <Link to="/products" className="bg-white text-stone-900 px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-teal-50 transition-colors">Shop Sustainably</Link>
            </div>
        </div>
