...
import { 
  User, Store, ShoppingBag, MapPin, Heart, Star, Settings as SettingsIcon, 
  Menu, X, Camera, Edit2, LogOut, ChevronRight, Package 
} from 'lucide-react';

import { updateUserProfile } from '../features/auth/authSlice';
// Changed import from MyMeasurements to SellerProfile (assuming previous step updated component name)
import SellerProfile from '../pages/MyMeasurements'; 
import MyOrders from '../pages/MyOrdersPage';
import MyAddresses from '../pages/MyAddresses';
import MyWishlist from './MyWishlist';
import MyReviews from './MyReviews'; // Ensure this matches file name
...
const Settings = () => (
  <div className="bg-white border border-stone-200 p-8 rounded-sm text-center">
    ...
    <p className="text-stone-500 mt-2 text-sm font-light">Manage your password and notification preferences.</p>
  </div>
);
...
const DashboardStats = () => {
  ...
  return (
    // Changed bg to Stone/Teal gradient
    <div className="bg-stone-900 text-white p-8 rounded-sm shadow-xl mb-8 relative overflow-hidden">
      {/* Decorative Glow - Changed color to Teal */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-teal-900/30 rounded-full blur-[80px] pointer-events-none"></div>
      
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          {/* Changed text color to Teal */}
          <span className="text-teal-500 text-xs font-bold uppercase tracking-widest mb-2 block">Seller Dashboard</span>
          ...
          <p className="text-stone-400 mt-2 font-light text-sm max-w-lg">
            Manage your listings, track your sales, and update your public seller profile.
          </p>
        </div>
        ...
        <div className="flex gap-4">
           <div className="bg-white/10 backdrop-blur-sm p-4 rounded-sm border border-white/10 text-center min-w-[100px]">
              ...
              <p className="text-[10px] uppercase tracking-widest text-stone-400">Active Listings</p>
           </div>
           <div className="bg-white/10 backdrop-blur-sm p-4 rounded-sm border border-white/10 text-center min-w-[100px]">
              <p className="text-2xl font-marcellus text-white">0</p>
              <p className="text-[10px] uppercase tracking-widest text-stone-400">Items Sold</p>
           </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white border border-stone-200 rounded-sm p-8 shadow-sm">
      <div className="flex justify-between items-center mb-8 border-b border-stone-100 pb-6">
        ...
        <button
          ...
          // Changed hover text color to Teal
          className="text-xs font-bold uppercase tracking-widest text-stone-500 hover:text-teal-700 flex items-center gap-2 transition-colors"
        >
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex-shrink-0 relative group">
          ...
          <button
            ...
            className="absolute bottom-0 right-0 bg-stone-900 text-white p-2.5 rounded-full hover:bg-teal-700 transition-colors shadow-md border-2 border-white"
            title="Change Photo"
          >
            <Camera size={16} />
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full p-3 bg-stone-50 border border-stone-200 rounded-sm text-stone-800 focus:outline-none focus:border-teal-700 focus:bg-white disabled:opacity-60 disabled:bg-transparent transition-all"
              />
            </div>
            <div>
          ...
          {isEditing && (
            <div className="pt-4 border-t border-stone-100 flex justify-end">
              <button
                type="submit"
                // Changed hover bg to Teal
                className="bg-stone-900 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-teal-700 rounded-sm transition-colors shadow-lg"
              >
                Save Changes
              </button>
            </div>
...
const ProfilePage = () => {
  ...
  const navLinks = [
    ...
    // Replaced 'Measurements' (Ruler) with 'Seller Profile' (Store)
    { path: '/profile/measurements', label: 'Seller Profile', icon: Store }, 
    { path: '/profile/orders', label: 'My Purchases', icon: Package }, // Changed label to Purchases
    { path: '/profile/addresses', label: 'Addresses', icon: MapPin },
    { path: '/profile/wishlist', label: 'Saved Items', icon: Heart }, // Changed label to Saved Items
    { path: '/profile/reviews', label: 'Feedback', icon: Star }, // Changed label to Feedback
                            ...
                            <li key={link.path}>
                                <Link
                                    ...
                                    className={`flex items-center gap-3 px-6 py-4 text-sm font-medium transition-all border-l-4 ${
                                        active 
                                            // Changed active styles to Teal
                                            ? 'border-teal-700 bg-teal-50/10 text-teal-900' 
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    <Routes>
                        ...
                        {/* Note: mapped 'measurements' path to SellerProfile component */}
                        <Route path="measurements" element={<SellerProfile />} />
